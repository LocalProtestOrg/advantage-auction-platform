...
## Marketplace Integrity Standard (Phase 6A — permanent)
There is ONE authoritative source of public marketplace data: Importer → canonical DB → canonical public APIs (`/api/public/*`) → ALL public experiences. No page may bypass the canonical public API, query DB tables directly for a public surface, maintain separate filtering/count logic, or apply different publication rules. Visibility is defined in exactly one place — `src/lib/marketplaceVisibility.js` (`activeEventSql`, `activeNativeAuctionSql`, `canonicalCounts`) — import it, never re-implement it. Any new public listing surface must be added to the Marketplace Integrity Suite (`src/services/marketplaceIntegrity.js`) with a count expectation from `canonicalCounts`. The deployment gate `npm run gate` (jest + integrity suite) must be GREEN to deploy; a second retrieval path will diverge and fail it. Full standard: `docs/architecture/marketplace-integrity-architecture.md`.