#!/usr/bin/env node
/* stg-seed-summer-showcase.js — STAGING-guarded. Seeds a clearly-labeled
 * "Coming Soon / Demo Showcase" upcoming auction so the marketplace has a polished
 * representative catalog to show. Idempotent (fixed UUIDs, ON CONFLICT DO NOTHING).
 *
 * SAFETY / DESIGN:
...
(async () => {
  ...
  if (raw.includes('ep-proud-leaf-an8pzkib')) { console.error('REFUSE: PRODUCTION endpoint detected (STAGING-only script).'); return 2; }
  if (!raw.includes('ep-royal-dawn-anarou3f')) { console.error('REFUSE: not the STAGING endpoint. Aborting.'); return 2; }
