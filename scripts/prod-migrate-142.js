#!/usr/bin/env node
/* prod-migrate-142.js — PRODUCTION-guarded apply of ONLY 142_marketing_director_ledger.sql.
   Additive: marketing_director_decisions + marketing_paid_allocations(+entries). NOTHING charges/sends/
   spends/flips a gate. Idempotent, non-destructive. */
const fs = require('fs'); const path = require('path'); const { Pool } = require('pg');
const FILE = '142_marketing_director_ledger.sql';
...
const verify = async (c) => (await c.query(`
  SELECT
    (SELECT count(*)::int FROM information_schema.tables WHERE table_name IN
      ('marketing_director_decisions','marketing_paid_allocations','marketing_paid_allocation_entries')) AS tbls,
...
(async () => {
  ...
  try {
    ...
    if (await ledgerHas(c)) { console.log('SKIP apply (already recorded; idempotent). Verifying only.'); }
      ...
      await c.query('BEGIN');
      try { await c.query(sql); await c.query('INSERT INTO schema_migrations (filename) VALUES ($1)', [FILE]); await c.query('COMMIT'); console.log('APPLIED 142 and recorded its ledger row.'); }
    ...
    const pass = recorded && v.tbls === 3 && v.a7_on === false && v.google_on === false;
