#!/usr/bin/env node
/* prod-migrate-166.js — PRODUCTION-guarded apply of ONLY 166_scoped_approval_check_fix.sql.
...
const FILE = '166_scoped_approval_check_fix.sql';
...
(async () => {
  ...
  try {
    ...
    if (done) console.log('SKIP apply (already recorded). Verifying only.');
      ...
      try { await c.query(fs.readFileSync(FILE_PATH, 'utf8')); await c.query('INSERT INTO schema_migrations (filename) VALUES ($1)', [FILE]); console.log('APPLIED 166.'); }
