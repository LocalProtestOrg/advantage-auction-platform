#!/usr/bin/env node
/*
 * stg-migrate-074.js — STAGING-guarded apply of ONLY 074_agreement_review_ack.sql.
 * Idempotent; verifies the two new agreement_signatures columns.
 *   railway run --service advantage-staging node scripts/stg-migrate-074.js
 */
const fs = require('fs');
...
const verify = async (c) => (await c.query(`
  SELECT (SELECT count(*) FROM information_schema.columns
            WHERE table_name='agreement_signatures'
              AND column_name IN ('reviewed_acknowledged','reviewed_acknowledged_at'))::int AS cols
`)).rows[0];
  ...
  try {
    if (await ledgerHas(c)) {
      ...
      try {
        ...
        console.log('APPLIED 074 and recorded its ledger row.');
