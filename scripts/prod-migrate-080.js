#!/usr/bin/env node
/* prod-migrate-080.js — PRODUCTION-guarded apply of ONLY 080_directory_mirror_fields.sql. DO NOT run without approval.
 *   railway run --service advantage-auction-platform --environment production node scripts/prod-migrate-080.js */
const fs = require('fs'); const path = require('path'); const { Pool } = require('pg');
const FILE = '080_directory_mirror_fields.sql';
...
const verify = async (c) => (await c.query(`
  SELECT (SELECT count(*) FROM information_schema.columns WHERE table_name='organizations'
            AND column_name IN ('description','lat','lng','google_place_id','bd_metadata'))::int AS cols
`)).rows[0];
...
(async () => {
  ...
  try {
    ...
    if (await ledgerHas(c)) { console.log('SKIP apply (already recorded; idempotent). Verifying only.'); }
      ...
      await c.query('BEGIN');
      try { await c.query(sql); await c.query('INSERT INTO schema_migrations (filename) VALUES ($1)', [FILE]); await c.query('COMMIT'); console.log('APPLIED 080 and recorded its ledger row.'); }
    ...
    line(); console.log('POST ledger[080]: ' + (recorded ? 'recorded' : 'NOT recorded') + ' | cols(5): ' + v.cols);
    const pass = recorded && v.cols === 5;
