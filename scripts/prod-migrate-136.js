#!/usr/bin/env node
/* prod-migrate-136.js — PRODUCTION-guarded apply of ONLY 136_behavioral_fuel_baselines_4h.sql.
   Additive: marketing_baselines snapshot store + refresh/watcher config. NOTHING connects a provider,
   activates A7, sends, spends, or imports a list. Idempotent, non-destructive. */
const fs = require('fs'); const path = require('path'); const { Pool } = require('pg');
const FILE = '136_behavioral_fuel_baselines_4h.sql';
const FILE_PATH = path.join(__dirname, '..', 'db', 'migrations', FILE);
const PROD_EP = 'ep-proud-leaf-an8pzkib'; const STG_EP = 'ep-royal-dawn-anarou3f';
const ledgerHas = async (c) => (await c.query('SELECT 1 FROM schema_migrations WHERE filename=$1', [FILE])).rowCount > 0;
const verify = async (c) => (await c.query(`
  SELECT
    (SELECT count(*)::int FROM information_schema.tables WHERE table_name='marketing_baselines') AS baselines_tbl,
    (SELECT count(*)::int FROM platform_config WHERE key IN
       ('marketing.refresh.platform_fact_minutes','marketing.refresh.behavioral_minutes','marketing.watcher_ending_soon.enabled')) AS cfg,
    (SELECT (value::text)::boolean FROM platform_config WHERE key='marketing.watcher_ending_soon.enabled') AS watcher_on,
...
(async () => {
  ...
  try {
    ...
    if (await ledgerHas(c)) { console.log('SKIP apply (already recorded; idempotent). Verifying only.'); }
      ...
      await c.query('BEGIN');
      try { await c.query(sql); await c.query('INSERT INTO schema_migrations (filename) VALUES ($1)', [FILE]); await c.query('COMMIT'); console.log('APPLIED 136 and recorded its ledger row.'); }
    ...
    const pass = recorded && v.baselines_tbl === 1 && v.cfg === 3 && v.watcher_on === true
