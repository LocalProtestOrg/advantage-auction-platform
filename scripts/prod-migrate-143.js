#!/usr/bin/env node
/* prod-migrate-143.js — PRODUCTION-guarded apply of ONLY 143_marketing_creative_jobs.sql.
   Additive: marketing_creative_jobs + marketing_creative_provenance. NOTHING sends/publishes/spends/flips a
   gate. Idempotent, non-destructive. */
const fs = require('fs'); const path = require('path'); const { Pool } = require('pg');
const FILE = '143_marketing_creative_jobs.sql';
const FILE_PATH = path.join(__dirname, '..', 'db', 'migrations', FILE);
const PROD_EP = 'ep-proud-leaf-an8pzkib'; const STG_EP = 'ep-royal-dawn-anarou3f';
const ledgerHas = async (c) => (await c.query('SELECT 1 FROM schema_migrations WHERE filename=$1', [FILE])).rowCount > 0;
const verify = async (c) => (await c.query(`
  SELECT
    (SELECT count(*)::int FROM information_schema.tables WHERE table_name IN ('marketing_creative_jobs','marketing_creative_provenance')) AS tbls,
    (SELECT count(*)::int FROM information_schema.columns WHERE table_name='marketing_creative_provenance' AND column_name IN ('rgb_edited','generative','fidelity')) AS prov_cols,
...
(async () => {
  ...
  try {
    ...
    if (await ledgerHas(c)) { console.log('SKIP apply (already recorded; idempotent). Verifying only.'); }
      ...
      await c.query('BEGIN');
      try { await c.query(sql); await c.query('INSERT INTO schema_migrations (filename) VALUES ($1)', [FILE]); await c.query('COMMIT'); console.log('APPLIED 143 and recorded its ledger row.'); }
    ...
    const pass = recorded && v.tbls === 2 && v.prov_cols === 3 && v.a7_on === false && v.google_on === false;
