#!/usr/bin/env node
/*
 * stg-migrate-076.js — STAGING-guarded apply of ONLY 076_organizations_and_events.sql.
 * Idempotent (schema_migrations ledger); additive; verifies the 7 new tables + seeds.
 *   railway run --service advantage-staging node scripts/stg-migrate-076.js
 */
const fs = require('fs');
  ...
  try {
    if (await ledgerHas(c)) {
      ...
      try {
        ...
        console.log('APPLIED 076 and recorded its ledger row.');
