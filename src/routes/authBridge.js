...
/**
 * /auth/bd/* — BD → Advantage.Bid identity bridge (Option 2, JWT-seed). Mounted by server.js ONLY
 * when IDENTITY_BRIDGE_ENABLED === 'true'. Reuses the existing login JWT + users table; does NOT
 * modify existing auth. Identity only — never grants seller/org-owner/admin. Logs are production-safe:
 * accepted/rejected + a non-sensitive reason category only — never a code, secret, hash, or email.
 */
...
router.post('/api/auth/bd/exchange', express.json({ limit: '8kb' }), limiter, async (req, res) => {
  try {
    ...
    console.log('[identity-bridge] exchange:', out.status === 200 ? 'accepted' : ('rejected (' + (out.json && out.json.error) + ')'));
